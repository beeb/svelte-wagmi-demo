<script lang="ts">
  ...
  async function getBalance(token: string, account: string) {
    ...
    if (web3.network?.chainNamespace === 'solana' && web3.solConnection) {
      ...
      const tokenAccount = (
        await web3.solConnection.getTokenAccountsByOwner(new PublicKey(account), {
          mint: tokenAddress
        })
      ).value.at(0)
      if (!tokenAccount) {
      ...
      const balance = await web3.solConnection.getTokenAccountBalance(tokenAccount.pubkey)
      return {
