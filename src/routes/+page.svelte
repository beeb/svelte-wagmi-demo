<script lang="ts">
  ...
  const balance = $derived(
    usdtAddress && web3.address ? getBalance(usdtAddress, web3.address) : { balance: 0n, decimals: 6 }
  )
  ...
  async function getBalance(token: string, account: string) {
    ...
    if (web3.network?.chainNamespace === 'eip155' && isAddress(token) && isAddress(account)) {
    ...
    if (web3.solRpc) {
      console.log(web3.solRpc)
      const { value: tokenAccounts } = await web3.solRpc
        .getTokenAccountsByOwner(address(account), {
          mint: address(token)
        })
        .send()
      ...
      const balance = await web3.solRpc.getTokenAccountBalance(tokenAccount.pubkey).send()
      return {
